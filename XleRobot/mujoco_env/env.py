...
class XLeRobotController:
    ...
    """ 输入数据加载器 """
    def start_replay(self, dataloader):
        self.replay_iter = iter(dataloader)

    """ 设定运行模式 collect 收集数据 replay 重播数据集 """
    ...
    """ 读取按键输出状态，返回 key_states[key_name]"""
    ...
    def run(self):
        ...
        while self.viewer.is_alive:
            ...
            self.update_keyboards() # 获取键盘状态，存在 key_states[key_name]

            if self.mode == "collect":
                self.update_reference() # 键盘输入转运动参考量
                self.update_control() # 把期望运动映射给 Mujoco 的控制信号

                if self.recording:
                    state = self.qFb[:18].copy()
